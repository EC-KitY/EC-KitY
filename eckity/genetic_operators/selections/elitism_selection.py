...
class ElitismSelection(SelectionMethod):
    def __init__(self, num_elites=1, higher_is_better=False, events=None):
        super().__init__(events=events, higher_is_better=higher_is_better, arity=num_elites)
        self.higher_is_better = higher_is_better

    def select(self, source_inds, dest_inds):
        elites = sorted(source_inds,
                        key=lambda ind: ind.get_augmented_fitness(),
                        reverse=self.higher_is_better)[:self.arity]
        for elite in elites:
            cloned = elite.clone()
