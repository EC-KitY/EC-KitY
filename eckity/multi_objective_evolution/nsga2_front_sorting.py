...
class NSGA2FrontSorting:
    """
    This class is incharge of setting the value of
    selecting only the best individuals out of the population
    (meaning that the are dominated by as little
    amount of other Individuals as possible)

    this class also sets the values of the
    "front_rank" and"crowding" of each individual
    """

    def select_for_population(
        self, population: Population, new_pop_size: Optional[int] = None
    ) -> None:
        for sub_population in population.sub_populations:
            dest_inds: List[Individual] = []
            dest_inds = self._select(
                sub_population.individuals, dest_inds, new_pop_size
            )
            sub_population.individuals = dest_inds

    def _select(
        self,
        source_inds: List[Individual],
        dest_inds: List[Individual],
        pop_size: int,
    ):
        """
        ...
        while len(dest_inds) < pop_size:
            ...
            if total_pareto_size > pop_size:
                ...
                new_pareto_front.sort(
                    key=lambda x: x.fitness.crowding, reverse=True
                )
                new_pareto_front = new_pareto_front[
                    : int(number_solutions_needed)
                ]  # take the individuals with the largest crowding
            dest_inds += new_pareto_front
            source_inds = self._remove_pareto_front(
                source_inds, new_pareto_front
            )
            front_rank += 1
        return dest_inds
    ...
    def _remove_pareto_front(self, source_inds, pareto_front):
        for dominating_ind in pareto_front:
            for dominated_ind in self.domination_dict[
                dominating_ind
            ].dominates:
                self.domination_dict[dominated_ind].domination_counter -= 1
        return list(set(source_inds) - set(pareto_front))

    def _update_new_pareto_front_rank(
        self, new_pareto_front: List[Individual], front_rank: int
    ):
    ...
    def _calc_fronts_crowding(self, front: List[Individual]):
        ...
        for objective_index in objectiv_indexes:
            front.sort(
                key=lambda x: x.get_pure_fitness()[objective_index]
            )  # sort for each objectiv
            front[0].fitness.crowding = float("inf")
            front[-1].fitness.crowding = float("inf")
            for i in range(1, len(front) - 1):
                curr_crowding = (
                    front[i + 1].get_pure_fitness()[objective_index]
                    - front[i - 1].get_pure_fitness()[objective_index]
                )
                d = (
                    front[-1].get_pure_fitness()[objective_index]
                    - front[0].get_pure_fitness()[objective_index]
                )
                if d:
                    curr_crowding /= d
    ...
    def _pareto_front_finding(self, source_inds: List[Individual]):
        pareto_front = [
            ...
            if self.domination_dict[ind].domination_counter == 0
        ]
        return pareto_front
