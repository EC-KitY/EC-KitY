...
class NSGA2Fitness(Fitness):
    def __init__(
        ...
        self.fitness: List[float] = fitness  # list of objectives
        self.crowding = 0
        self.front_rank = float("inf")
        if self.fitness is not None and type(self.higher_is_better) is bool:
    ...
    def check_comparable_fitnesses(
        self, other_fitness: Fitness, ind: Individual, other_ind: Individual
    ):
        if not isinstance(other_fitness, NSGA2Fitness):
            raise TypeError(
                "Expected NSGA2Fitness object in better_than, got", type(other_fitness)
            )
        ...
        if len(other_fitness.get_augmented_fitness(other_ind)) != len(
    ...
    def equal_to(self, ind, other_fitness, other_ind):
        return (
            ...
            and self.crowding == other_ind.fitness.crowding
        )
    ...
    def dominate(self, ind, other_fitness, other_ind):
        ...
        return all(
            [
                self._o1_at_least_o2(o1, o2, h_is_b)
                for h_is_b, o1, o2 in zip(self.higher_is_better, self_fit, other_fit)
            ]
        ) and any(
            [
                self._o1_better_then_o2(o1, o2, h_is_b)
                for h_is_b, o1, o2 in zip(self.higher_is_better, self_fit, other_fit)
            ]
        )
